...
<head>
    ...
    <script>
        ...
        function onbuttonclick(pageno){
                ...
                if(xmlhttp.readyState==4&&xmlhttp.status==200){
                    ...
                    if(jsonArray.length>0){
                        for(var i=0;i<jsonArray.length;i++){
                            var jsonObjectQuestion=jsonArray[i];
                            innerhtml=innerhtml+"<tr><td>"
                                +jsonObjectQuestion.subject+"</td><td>"
                                +jsonObjectQuestion.answer+"</td>"+
                                // "<td><a href='#' onclick='return clickEdit("+jsonObjectQuestion.id+jsonObjectQuestion.subject+jsonObjectQuestion.answer+")'>编辑</a></td>" +
                                "<td><a href='#' onclick='return clickEdit("+jsonObjectQuestion.id+")'>编辑</a></td>" +
                                "<td><a href='#' onclick='return clickDelete("+jsonObjectQuestion.id+")'>删除</a></td>"+
                                "</tr>";
                        }
                    }
                    //把这一页商品展示出来
            ...
            var url="../question/showQuestionPage";
        ...
        function clickDelete(id){
            ...
            var url="../question/deleleone?id="+id;
            xmlhttp.open("get",url);
            xmlhttp.send();
...
试题管理：<button onclick="addQuestion()">添加试题</button>
<br>
<div>
